...
(function () {
	...
	if (isPreview || !CSCC.hasInteracted()) {
		banner.style.display = '';
		if (overlay) overlay.style.display = '';
		CSCC.log(isPreview ? 'Banner: showing (preview mode)' : 'Banner: showing (no prior consent)');
		...
		CSCC.log('Banner: hidden (consent already stored)');
	}

	// Expose openBanner() on the public API so it can be called from JS at any time.
	CSCC.openBanner = function () {
		banner.style.display = '';
		if (overlay) overlay.style.display = '';
		banner.focus();
		CSCC.log('Banner: opened via API');
	};
	...
	document.addEventListener('cscc:consentUpdated', function () {
		...
		CSCC.log('Banner: hidden after consent update');
	});
	...
	if (btnAccept) {
		btnAccept.addEventListener('click', function () {
			CSCC.log('Banner: Accept All clicked');
			CSCC.acceptAll();
		});
	}
	...
	if (btnDeny) {
		btnDeny.addEventListener('click', function () {
			CSCC.log('Banner: Deny All clicked');
			CSCC.denyAll();
		});
	}
	...
	if (btnPrefs) {
		btnPrefs.addEventListener('click', function () {
			CSCC.log('Banner: Preferences clicked');
			document.dispatchEvent(new CustomEvent('cscc:openPreferences'));
		});
	}
	...
	if (prefIcon) {
		if (CSCC.hasInteracted()) {
			prefIcon.style.display = '';
			CSCC.log('Preferences icon: visible (consent already stored)');
		}
		document.addEventListener('cscc:consentUpdated', function () {
			prefIcon.style.display = '';
			CSCC.log('Preferences icon: visible after consent update');
		});
	}
